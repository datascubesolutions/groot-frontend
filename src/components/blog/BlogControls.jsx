        ...
        <div className="sticky top-20 z-40 w-full">
            <div className="w-full max-w-7xl mx-auto">
                <div className="bg-background/80 backdrop-blur-xl border border-border/60 rounded-2xl p-2 shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col md:flex-row items-center gap-4">
                    ...
                    <div className="hidden md:flex flex-1 items-center gap-1.5 overflow-x-auto no-scrollbar mask-linear-fade">
                        <AnimatePresence mode="popLayout">
                            {categories.map((category) => (
                                <button
                                    ...
                                    className={cn(
                                        "relative px-4 py-2.5 text-sm font-medium transition-all duration-200 rounded-xl whitespace-nowrap select-none ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forest/40 focus-visible:ring-offset-2",
                    ...
                    <div className="flex md:hidden w-full overflow-x-auto pb-2 gap-2 mask-linear-fade no-scrollbar">
                        {categories.map((cat) => (
                            <button
                                key={cat}
                                onClick={() => onSelectCategory(cat)}
                                className={cn(
                                    "whitespace-nowrap px-4 py-2.5 rounded-full text-sm font-medium transition-all duration-200 border shadow-sm",
