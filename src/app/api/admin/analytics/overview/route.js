...
export async function GET(req) {
  try {
    ...
    if (!propertyId || !process.env.GA_CLIENT_EMAIL || !process.env.GA_PRIVATE_KEY) {
      return NextResponse.json({ error: "Analytics API not configured. Missing environment variables." }, { status: 500 });
    }
    ...
    const trends = trendResponse.rows?.map(row => {
      ...
      return {
        date: `${y}-${m}-${d}`,
        displayDate: new Date(`${y}-${m}-${d}T00:00:00`).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
    ...
    const irrelevantEvents = ["page_view", "session_start", "user_engagement", "first_visit", "scroll", "click", "view_search_results"];
    let events = [];
    if (eventResponse.rows) {
       events = eventResponse.rows
         .filter(row => !irrelevantEvents.includes(row.dimensionValues[0].value) && !row.dimensionValues[0].value.startsWith('scroll_'))
         .slice(0, 5)
         .map(row => ({
