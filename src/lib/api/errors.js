...
export function handleApiError(error) {
  ...
  if (error.name === "AbortError") {
    return new ApiError("Request was cancelled.", 0, { type: "abort" });
  }
...
export function getErrorMessage(error) {
  if (error instanceof ApiError) {
    ...
    if (
      error.data?.details?.validationErrors &&
      Array.isArray(error.data.details.validationErrors)
    ) {
  ...
  if (
    error?.error?.details?.details?.validationErrors &&
    Array.isArray(error.error.details.details.validationErrors)
  ) {
  ...
  if (
    error?.details?.details?.validationErrors &&
    Array.isArray(error.details.details.validationErrors)
  ) {
...
const errorsApi = { ApiError, handleApiError, getErrorMessage, isApiError };
